    ...
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
      <section className="hero-field grid items-center gap-10 lg:grid-cols-[1.4fr_1fr]">
        <div>
          ...
          <div className="fade-in mt-8 flex flex-wrap gap-3 [animation-delay:180ms]">
            ...
            <Link
              href="/history"
              className="press inline-flex items-center gap-2 rounded-lg border border-line bg-surface px-5 py-2.5 font-medium text-ink hover:bg-canvas focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-brand-600"
            >
      ...
      <section aria-labelledby="pipeline" className="mt-20">
        ...
        <ol className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {PIPELINE.map(({ icon: Icon, title, text }, index) => (
            <Reveal as="li" key={title} index={index} className="lift rounded-xl border border-line bg-surface p-5">
      ...
      <section aria-labelledby="outcomes" className="mt-20">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          {OUTCOMES.map(({ icon: Icon, tone, title, text }, index) => (
            <Reveal key={title} index={index} className={`lift rounded-xl border p-5 ${tone}`}>
      ...
      <section aria-labelledby="boundary" className="mt-20 grid gap-4 md:grid-cols-2">
        ...
        <Reveal index={1} className="rounded-xl border border-line bg-surface p-6">
